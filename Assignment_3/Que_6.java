...
public class Que_6 {
    public static void main(String[] args) {
        ...
        buyedRoses(prices, totalMoney);
    }
    public static void buyedRoses(int[] prices,int totalMoney){
        for (int i = 0; i < prices.length; i++) {
            for (int j = i+1; j < prices.length; j++) {
                if(prices[i]+prices[j]==totalMoney){
                    System.out.println("Deepak should buy roses whose prices are " + prices[i] +" and" + prices[j]+".");
                }
            }
        }
    }
