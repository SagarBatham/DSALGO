...
public class Help_Ramu {
    public static void main(String[] args) {
        ...
        int noRiks=sc.nextInt();
        int noCabs=sc.nextInt();

        int[] riks=new int[noRiks];
        int[] cabs=new int[noCabs];

        for (int i = 0; i < riks.length; i++) {
            riks[i]=sc.nextInt();
        }
        ...
        for (int i = 0; i < riks.length; i++) {
            riksCost+=Math.min(riks[i]*c1, c2);
        }

        for (int i = 0; i < cabs.length; i++) {
            cabCost+=Math.min(cabs[i]*c1, c2);
        }

        int riksTotalcost=Math.min(riksCost, c3);
        int cabTotalcost=Math.min(cabCost, c3);

        int Total=riksTotalcost+cabTotalcost;
