...
// Deepak has a limited amount of money that he can spend on his girlfriend. So he decides to buy two roses for her. Since roses are of varying 
// sizes, their prices are different. Deepak wishes to completely spend that fixed amount of money on buying roses for her.
...
// NOTE: If there are multiple solutions print the solution that minimizes the difference between the prices i and j. After each test case, you
//  must print a blank line.
...
public class Pair_rose {
    public static void main(String[] args) {
        ...
        int t=sc.nextInt();
        while(t-->0){
            int avRose=sc.nextInt();
            int[] pricerose=new int[avRose];
            for (int i = 0; i < pricerose.length; i++) {
                pricerose[i]=sc.nextInt();
            }
            int first=0;
            int second=0;
            int Avmoney=sc.nextInt();
            int diff=Integer.MAX_VALUE;
            for (int i = 0; i < pricerose.length; i++) {
                for (int j = i+1; j < pricerose.length; j++) {
                if(pricerose[i]+pricerose[j]==Avmoney){
                    int differ=Math.abs(pricerose[i]-pricerose[j]);
                    if(differ<diff){
                        diff=differ;
                        first=Math.min(pricerose[i], pricerose[j]);
                        second=Math.max(pricerose[i], pricerose[j]); 
                    }
                }
                  
            }
            }
            System.out.print("Deepak should buy roses whose prices are "+ first+ " and "+second+ "."); 
            System.out.println();
        }
    }
}